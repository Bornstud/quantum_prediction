import psycopg2
import psycopg2.extras
from psycopg2 import pool
import os
from datetime import datetime
from typing import Optional, Dict, List, Any
import json

class DatabaseManager:
    """Manages database operations for the BCI application with PostgreSQL"""
    
    _connection_pool = None
    
    def __init__(self, db_url: str = None):
        self.db_url = db_url or os.getenv('DATABASE_URL')
        if not self.db_url:
            raise ValueError("DATABASE_URL environment variable not set")
        
        # Initialize connection pool once
        if DatabaseManager._connection_pool is None:
            try:
                DatabaseManager._connection_pool = psycopg2.pool.SimpleConnectionPool(
                    minconn=1,
                    maxconn=10,
                    dsn=self.db_url
                )
            except Exception as e:
                print(f"Error creating connection pool: {e}")
                DatabaseManager._connection_pool = None
    
    def get_connection(self):
        """Get PostgreSQL database connection from pool"""
        if DatabaseManager._connection_pool:
            try:
                return DatabaseManager._connection_pool.getconn()
            except:
                # Fallback to direct connection if pool fails
                return psycopg2.connect(self.db_url)
        else:
            return psycopg2.connect(self.db_url)
    
    def return_connection(self, conn):
        """Return connection to pool"""
        if DatabaseManager._connection_pool and conn:
            try:
                DatabaseManager._connection_pool.putconn(conn)
            except:
                self.return_connection(conn)
    
    def initialize_database(self):
        """Initialize PostgreSQL database tables"""
        conn = self.get_connection()
        cursor = conn.cursor()
        
        # Users table
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS users (
                id SERIAL PRIMARY KEY,
                username TEXT UNIQUE NOT NULL,
                password_hash TEXT NOT NULL,
                full_name TEXT NOT NULL,
                email TEXT UNIQUE NOT NULL,
                role TEXT NOT NULL,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                last_login TIMESTAMP
            )
        ''')
        
        # Sessions table
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS sessions (
                id SERIAL PRIMARY KEY,
                user_id INTEGER NOT NULL REFERENCES users(id),
                filename TEXT NOT NULL,
                upload_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                channels_original INTEGER,
                channels_selected INTEGER,
                processing_status TEXT
            )
        ''')
        
        # Predictions table
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS predictions (
                id SERIAL PRIMARY KEY,
                session_id INTEGER NOT NULL REFERENCES sessions(id),
                model_type TEXT NOT NULL,
                model_name TEXT NOT NULL,
                accuracy REAL,
                prediction_result TEXT,
                processing_time REAL,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        ''')
        
        # Brain metrics table
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS brain_metrics (
                id SERIAL PRIMARY KEY,
                session_id INTEGER NOT NULL REFERENCES sessions(id),
                alpha_power REAL,
                beta_power REAL,
                theta_power REAL,
                delta_power REAL,
                total_power REAL,
                metrics_json TEXT
            )
        ''')
        
        # Activity logs table
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS activity_logs (
                id SERIAL PRIMARY KEY,
                user_id INTEGER NOT NULL REFERENCES users(id),
                action TEXT NOT NULL,
                details TEXT,
                timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        ''')
        
        # EEG data table (encrypted storage)
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS eeg_data (
                id SERIAL PRIMARY KEY,
                session_id INTEGER NOT NULL REFERENCES sessions(id),
                data_encrypted BYTEA,
                data_hash TEXT
            )
        ''')
        
        # Password reset tokens table
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS password_reset_tokens (
                id SERIAL PRIMARY KEY,
                user_id INTEGER NOT NULL REFERENCES users(id),
                token TEXT UNIQUE NOT NULL,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                expires_at TIMESTAMP NOT NULL,
                used INTEGER DEFAULT 0
            )
        ''')
        
        conn.commit()
        cursor.close()
        self.return_connection(conn)
    
    def create_user(self, username: str, password_hash: str, full_name: str, 
                   email: str, role: str) -> Optional[int]:
        """Create a new user"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO users (username, password_hash, full_name, email, role)
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id
            ''', (username, password_hash, full_name, email, role))
            user_id = cursor.fetchone()[0]
            conn.commit()
            cursor.close()
            self.return_connection(conn)
            return user_id
        except psycopg2.IntegrityError:
            return None
    
    def get_user_by_username(self, username: str) -> Optional[Dict]:
        """Get user by username"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute('SELECT * FROM users WHERE username = %s', (username,))
        row = cursor.fetchone()
        cursor.close()
        self.return_connection(conn)
        
        if row:
            return dict(row)
        return None
    
    def update_last_login(self, user_id: int):
        """Update user's last login time"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            UPDATE users SET last_login = CURRENT_TIMESTAMP WHERE id = %s
        ''', (user_id,))
        conn.commit()
        cursor.close()
        self.return_connection(conn)
    
    def create_session(self, user_id: int, filename: str, channels_original: int, 
                      channels_selected: int) -> int:
        """Create a new analysis session"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO sessions (user_id, filename, channels_original, 
                                channels_selected, processing_status)
            VALUES (%s, %s, %s, %s, 'processing')
            RETURNING id
        ''', (user_id, filename, channels_original, channels_selected))
        session_id = cursor.fetchone()[0]
        conn.commit()
        cursor.close()
        self.return_connection(conn)
        return session_id
    
    def update_session_status(self, session_id: int, status: str):
        """Update session processing status"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            UPDATE sessions SET processing_status = %s WHERE id = %s
        ''', (status, session_id))
        conn.commit()
        cursor.close()
        self.return_connection(conn)
    
    def save_prediction(self, session_id: int, model_type: str, model_name: str,
                       accuracy: float, prediction_result: str, processing_time: float) -> int:
        """Save prediction results"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO predictions (session_id, model_type, model_name, accuracy,
                                   prediction_result, processing_time)
            VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING id
        ''', (session_id, model_type, model_name, accuracy, prediction_result, processing_time))
        pred_id = cursor.fetchone()[0]
        conn.commit()
        cursor.close()
        self.return_connection(conn)
        return pred_id
    
    def save_brain_metrics(self, session_id: int, alpha: float, beta: float,
                          theta: float, delta: float, total: float, metrics_dict: Dict):
        """Save brain metrics"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO brain_metrics (session_id, alpha_power, beta_power,
                                      theta_power, delta_power, total_power, metrics_json)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        ''', (session_id, alpha, beta, theta, delta, total, json.dumps(metrics_dict)))
        conn.commit()
        cursor.close()
        self.return_connection(conn)
    
    def save_eeg_data(self, session_id: int, data_encrypted: bytes, data_hash: str):
        """Save encrypted EEG data"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO eeg_data (session_id, data_encrypted, data_hash)
            VALUES (%s, %s, %s)
        ''', (session_id, data_encrypted, data_hash))
        conn.commit()
        cursor.close()
        self.return_connection(conn)
    
    def log_activity(self, user_id: int, action: str, details: str = ""):
        """Log user activity"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO activity_logs (user_id, action, details)
            VALUES (%s, %s, %s)
        ''', (user_id, action, details or ""))
        conn.commit()
        cursor.close()
        self.return_connection(conn)
    
    def get_predictions(self, session_id: int, model_type: Optional[str] = None) -> List[Dict]:
        """Get predictions for a session, optionally filtered by model type"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        if model_type:
            cursor.execute('''
                SELECT * FROM predictions WHERE session_id = %s AND model_type = %s
                ORDER BY created_at DESC
            ''', (session_id, model_type))
        else:
            cursor.execute('''
                SELECT * FROM predictions WHERE session_id = %s
                ORDER BY created_at DESC
            ''', (session_id,))
        rows = cursor.fetchall()
        cursor.close()
        self.return_connection(conn)
        return [dict(row) for row in rows]
    
    def get_user_sessions(self, user_id: int) -> List[Dict]:
        """Get all sessions for a user"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute('''
            SELECT * FROM sessions WHERE user_id = %s ORDER BY upload_time DESC
        ''', (user_id,))
        rows = cursor.fetchall()
        cursor.close()
        self.return_connection(conn)
        return [dict(row) for row in rows]
    
    def get_session_predictions(self, session_id: int) -> List[Dict]:
        """Get predictions for a session"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute('''
            SELECT * FROM predictions WHERE session_id = %s ORDER BY created_at
        ''', (session_id,))
        rows = cursor.fetchall()
        cursor.close()
        self.return_connection(conn)
        return [dict(row) for row in rows]
    
    def get_session_metrics(self, session_id: int) -> Optional[Dict]:
        """Get brain metrics for a session"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute('''
            SELECT * FROM brain_metrics WHERE session_id = %s
        ''', (session_id,))
        row = cursor.fetchone()
        cursor.close()
        self.return_connection(conn)
        
        if row:
            return dict(row)
        return None
    
    def get_recent_activity(self, user_id: int, limit: int = 10) -> List[Dict]:
        """Get recent activity logs for a user"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute('''
            SELECT * FROM activity_logs WHERE user_id = %s 
            ORDER BY timestamp DESC LIMIT %s
        ''', (user_id, limit))
        rows = cursor.fetchall()
        cursor.close()
        self.return_connection(conn)
        return [dict(row) for row in rows]
    
    def get_all_users(self) -> List[Dict]:
        """Get all users (admin only)"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute('SELECT id, username, full_name, email, role, created_at, last_login FROM users')
        rows = cursor.fetchall()
        cursor.close()
        self.return_connection(conn)
        return [dict(row) for row in rows]
    
    def get_user_session_count(self, user_id: int) -> int:
        """Get session count for a user"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute('SELECT COUNT(*) as count FROM sessions WHERE user_id = %s', (user_id,))
        count = cursor.fetchone()['count']
        cursor.close()
        self.return_connection(conn)
        return count
    
    def get_all_activity_logs(self, limit: int = 50) -> List[Dict]:
        """Get all activity logs (admin only)"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute('''
            SELECT al.*, u.username, u.full_name 
            FROM activity_logs al
            JOIN users u ON al.user_id = u.id
            ORDER BY al.timestamp DESC LIMIT %s
        ''', (limit,))
        rows = cursor.fetchall()
        cursor.close()
        self.return_connection(conn)
        return [dict(row) for row in rows]
    
    def create_password_reset_token(self, user_id: int, token: str, expires_at: str) -> int:
        """Create a password reset token"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO password_reset_tokens (user_id, token, expires_at)
            VALUES (%s, %s, %s)
            RETURNING id
        ''', (user_id, token, expires_at))
        token_id = cursor.fetchone()[0]
        conn.commit()
        cursor.close()
        self.return_connection(conn)
        return token_id
    
    def get_password_reset_token(self, token: str) -> Optional[Dict]:
        """Get password reset token by token string"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute('''
            SELECT * FROM password_reset_tokens 
            WHERE token = %s AND used = 0 AND expires_at > CURRENT_TIMESTAMP
        ''', (token,))
        row = cursor.fetchone()
        cursor.close()
        self.return_connection(conn)
        if row:
            return dict(row)
        return None
    
    def mark_token_used(self, token: str):
        """Mark password reset token as used"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            UPDATE password_reset_tokens SET used = 1 WHERE token = %s
        ''', (token,))
        conn.commit()
        cursor.close()
        self.return_connection(conn)
    
    def update_user_password(self, user_id: int, password_hash: str):
        """Update user password"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            UPDATE users SET password_hash = %s WHERE id = %s
        ''', (password_hash, user_id))
        conn.commit()
        cursor.close()
        self.return_connection(conn)
    
    def get_user_by_email(self, email: str) -> Optional[Dict]:
        """Get user by email"""
        conn = self.get_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute('SELECT * FROM users WHERE email = %s', (email,))
        row = cursor.fetchone()
        cursor.close()
        self.return_connection(conn)
        if row:
            return dict(row)
        return None
