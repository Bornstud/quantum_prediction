...
class ClassicalMLModels:
    ...
    def predict_svm(self, svm: SVC, X_test: np.ndarray, y_test: np.ndarray | None = None) -> Tuple[np.ndarray, float, Dict]:
        """Predict using SVM with optimized preprocessing"""
        start_time = time.time()
        ...
        if y_test is not None:
            ...
            metrics['precision'] = float(precision_score(y_test, predictions, average='weighted', zero_division='warn'))
            metrics['recall'] = float(recall_score(y_test, predictions, average='weighted', zero_division='warn'))
            metrics['f1_score'] = float(f1_score(y_test, predictions, average='weighted', zero_division='warn'))
    ...
    def predict_random_forest(self, rf: RandomForestClassifier, X_test: np.ndarray, 
        ...
        """Predict using Random Forest with optimized preprocessing"""
        start_time = time.time()
